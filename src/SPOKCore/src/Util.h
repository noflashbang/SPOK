...
#define SAFE_CAST_TO_UINT16(x) ((x) <= std::numeric_limits<uint16_t>::max() ? static_cast<uint16_t>(x) : throw std::overflow_error("size_t value is too large to fit in an uint16_t"))
#define SAFE_CAST_TO_UINT8(x)  ((x) <= std::numeric_limits<uint8_t>::max()  ? static_cast<uint8_t>(x)  : throw std::overflow_error("size_t value is too large to fit in an uint8_t"))