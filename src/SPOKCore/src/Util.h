...
#define SAFE_CAST_TO_UINT16(x) ((x) <= std::numeric_limits<uint16_t>::max() ? static_cast<uint16_t>(x) : throw std::overflow_error("size_t value is too large to fit in a uint16_t"))
